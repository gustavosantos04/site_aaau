...
import { parseEventTicketQrPayload, PortariaAuthError, validatePortariaQrTicketDto } from "@/lib/portaria";
...
export async function POST(request: Request, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  try {
    return NextResponse.json(await validatePortariaQrTicketDto(actor, eventId, parsed.qrToken));
  } catch (error) {
