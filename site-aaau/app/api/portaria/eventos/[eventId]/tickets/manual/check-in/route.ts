...
import { confirmPortariaManualTicket, PortariaAuthError } from "@/lib/portaria";
...
export async function POST(request: Request, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  try {
    return NextResponse.json(await confirmPortariaManualTicket(actor, eventId, ticketCode));
  } catch (error) {
